  ...
  <head>
	...
	<script language="javascript" type="text/javascript">
		...
		$(document).ready(function(){
			...
			var editIP = '${param.selText}';
			if (editIP.indexOf("-") >= 0) {
				$('#selIP').attr("checked","true");
				var Ipattr = editIP.split("-");
				$("#startip").val(Ipattr[0]);
				$("#endip").val(Ipattr[1]);
				selIPChecked();
			} else {
				selIPNoCheck();
		...
		function getSelIpText() {
			...
			for (i = objs.options.length - 1; i >= 0; i--) {
				if (objs.options[i].text != "") {
					var vdIp = objs.options[i].text;
					if (vdIp != '${param.selText}') {
