...
<html>
  <head>
	...
	<script type="text/javascript" src="<%=path%>/manager/common/js/window/window.js"></script>	
 	<script type="text/javascript" src="<%=path%>/manager/prjinfo/js/list.js"></script>	
	<script language="javascript" type="text/javascript">
		...
		$(function(){
			permEdit = '<view:AdminPermTag key="030203" path="<%=path%>" langKey="common_syntax_edit" type="1" />';
			customUserAdd = '<view:AdminPermTag key="030205" path="<%=path%>" langKey="adm_perm_030205" type="1" />';
		});
		
		function goback_(){
			if('<%=flag%>'==2){
				location.href='<%=path%>/manager/project/list.jsp?returnBack=1';
			}else{
				location.href='<%=path%>/manager/project/list.jsp';
			}
		}
		//添加监视人
		function addCustomUser(projId){
			projId = decodeURI(projId);
			window.location.href = contextPath + '/manager/project/addCustomUser.jsp?projId=' + encodeURI(encodeURI(projId))+"&ptype=1";
		}
	</script>
  </head>
  ...
  <tr>
	...
	<table width="800" border="0" cellspacing="0" cellpadding="0" style="margin-top:6px; margin-bottom:6px">
      <tr>
        <td width="116" align="right">定制信息摘要<view:LanguageTag key="colon"/></td>
        <td width="168"><input type="text" id="prjdesc" name="prjinfo.prjdesc" value="${prjinfo.prjdesc}" class="formCss100"/></td>
        <td width="116" align="right">项目名称<view:LanguageTag key="colon"/></td>
        <td width="168"><input type="text" id="prjname" name="prjinfo.prjname" value="${prjinfo.prjname}" class="formCss100"/></td>
        <td width="15"></td>
    ...
    <td width="39%" align="right" valign="top"><a href="javascript:addAdmPermCode('030201','<%=path%>');" class="isLink_HeiSe"><img src="<%=path%>/images/icon/rightarrow.png" hspace="6" vspace="6" title='<view:LanguageTag key="common_admin_keep_used"/>'/></a></td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="operTableBg">
      <tr>
      	<td width="5">&nbsp;&nbsp;</td>
        <td>
		    <view:AdminPermTag key="030204" path="<%=path%>" langKey="common_syntax_delete" type="2" />
