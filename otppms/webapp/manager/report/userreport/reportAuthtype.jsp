...
<html>
  <head>
	...
	function viewChart(){
		...
		var url = "<%=path%>/manager/report/reportAction!reportUserCountByAuthType.action?handler="+getExportReportHandler("<%=path%>")+"&domainAndOrgunitIds="+domainAndOrgunitIds+"&domainAndOrgName="+paramObj.orgunitNames;
		//创建统计图
    ...
    function exportReport(method,pngfileName){
        ...
        if(csvData!=""&&csvData!=undefined){
	      var url = "<%=path%>/manager/report/reportAction!"+method+".action?exportType=authType&csvData="+encodeURI(csvData)+"&fileName="+pngfileName+"&domainAndOrgunitIds="+paramObj.domainAndOrgunitIds+"&domainAndOrgName="+paramObj.orgunitNames;
	       $.post(url,
					function(msg){
					   ajaxbg.hide();
