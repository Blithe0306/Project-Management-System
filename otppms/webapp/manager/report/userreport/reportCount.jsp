...
<html>
  <head>
	...
	function viewChart(){
		...
		var url = "<%=path%>/manager/report/reportAction!reportUserCount.action?handler="+getExportReportHandler("<%=path%>")+"&domainAndOrgunitIds="+domainAndOrgunitIds+"&startdate="+startDate+"&enddate="+endDate+"&domainAndOrgName="+paramObj.orgunitNames;
		//创建统计图
    ...
    function exportReport(method,pngfileName){
        ...
        if(csvData!=""&&csvData!=undefined){
	      var url = "<%=path%>/manager/report/reportAction!"+method+".action?exportType=userCount&csvData="+encodeURI(csvData)+"&fileName="+pngfileName+"&domainAndOrgunitIds="+paramObj.domainAndOrgunitIds+"&domainAndOrgName="+paramObj.orgunitNames;
	       $.post(url,
					function(msg){
					   ajaxbg.hide();	
  ...
  <table width="800" border="0" cellspacing="0" cellpadding="0" style="margin-top:6px">
      <tr>
        ...
        <td width="100">&nbsp;&nbsp;&nbsp;&nbsp;
        	<span style="display:inline-block" class="query-button-css"><a href="#" onClick="viewChart()" class="isLink_LanSe" ><span><view:LanguageTag key="common_syntax_query"/></span></a></span></td>
      </tr>
    </table>
  </form>
	<hr class="hrLine" />
