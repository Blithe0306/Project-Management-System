...
function changeScroll(){
	...
	var getValue = body2.offsetWidth-body2.scrollWidth;
	getValue = Math.abs(getValue);
	var isHiddenScroll = false;
	if(navigator.userAgent.toUpperCase().indexOf('MSIE')!=-1){//是IE浏览器
		if(getValue==0||getValue==1||getValue==2||getValue==6||getValue==10||getValue==9||getValue==12){
			isHiddenScroll = true;
		}
	}
	if(navigator.userAgent.toUpperCase().indexOf('FIREFOX')!=-1){//火狐处理 差值为4
		if(getValue==4||getValue==1||getValue==7||getValue==9||getValue==8){
			isHiddenScroll = true;
        	}
    }
    if(navigator.userAgent.toUpperCase().indexOf('CHROME')!=-1){//chrome处理差值
    	if(getValue==3||getValue==8||getValue==4||getValue==6||getValue==1){
			isHiddenScroll = true;
        }
    }
	if(isHiddenScroll||body2.offsetWidth==body2.scrollWidth){
		$("#body2").css("overflow-x","hidden");
	}else{
		$("#body2").css("overflow-x","auto");
	}
}
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        rowClickSingleSelect: false,          // 单击行是否只能单选(后来添加 FT)
    ...
    $.ligerDefaults.GridString = {
        errorMessage: getLangVal('page_error_message',cPath),
        pageStatMessage: getLangVal('page_show_from',cPath)+' {from} '+getLangVal('page_to',cPath)+' {to}&nbsp;&nbsp;&nbsp;'+getLangVal('page_total',cPath)+' {total} '+getLangVal('page_records',cPath),
        pageTextMessage: 'Page',
        loadingMessage: getLangVal('page_loading',cPath),
        findTextMessage: getLangVal('page_find',cPath),
        noRecordMessage: getLangVal('page_no_match_records_exist',cPath),
        isContinueByDataChanged: getLangVal('page_whether_to_continue',cPath),
        cancelMessage: getLangVal('common_syntax_cancel',cPath),
        saveMessage: getLangVal('common_syntax_save',cPath),
        applyMessage: getLangVal('page_application',cPath),
        draggingMessage: '{count}'+getLangVal('page_row',cPath)
    };
    //接口方法扩展
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                      <div class='l-grid-body l-grid-body1' id='body1'>");
            gridhtmlarr.push("                      </div>");
            gridhtmlarr.push("                  </div>");
            ...
            gridhtmlarr.push("                          <div class='l-grid-header-inner'><table class='l-grid-header-table' cellpadding='0' cellspacing='0'><tbody></tbody></table></div>");
            gridhtmlarr.push("                      </div>");
            gridhtmlarr.push("                      <div class='l-grid-body l-grid-body2' id='body2'");
            gridhtmlarr.push("                      </div>");
            gridhtmlarr.push("                  </div>");
            ...
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnfirst'><span title='"+getLangVal('common_syntax_firstPage',contextPath)+"'></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnprev'><span title='"+getLangVal('common_syntax_prePage',contextPath)+"'></span></div>");
            gridhtmlarr.push("                </div>");
            //gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group' style='float: left;background: none;height: 24px;margin: -3px 5px;'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:40px' /> / <span></span></span></div>");
            //gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnnext'><span title='"+getLangVal('common_syntax_nextPage',contextPath)+"'></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnlast'><span title='"+getLangVal('common_syntax_lastPage',contextPath)+"'></span></div>");
            gridhtmlarr.push("                </div>");
            //gridhtmlarr.push("                <div class='l-bar-separator'></div>");
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnload'><span title='"+getLangVal('common_refresh',contextPath)+"'></span></div>");
            gridhtmlarr.push("                </div>");
           // gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        ...
        loadData: function (loadDataParm)
        {
            ...
            {
                g.loadServerData(param, clause, toParms);
                //g.loadServerData.ligerDefer(g, 10, [param, clause]);
            }
            g.loading = false;
        },
        loadServerData: function (param, clause, toParms)
        {
            var g = this, p = this.options;
            if(param.page) p.newPage = param.page;
            var formUser = param.source;
            //获取QueryForm参数
            if(formUser=='bindUser'){ //用户管理批量绑定区分用户、令牌的QueryForm参数
                   param = formParmsUser(p.newPage, p.pageSize);
            }else{
                if(toParms){
        		   param = formParms(p.newPage, p.pageSize);
        	    }
            }
            ...
            $.ajax(ajaxOptions);
            //如果 点击查询后 全选按钮仍是选中在此清除 zxh
            $(".l-grid-hd-row").removeClass('l-checked');
            //解决分页控件文本框在IE6下显示超出底层框的问题 XS
            if(!!window.ActiveXObject){//判断是IE浏览器
            	if(!window.XMLHttpRequest){//判断是IE6
            		$(".l-bar-group").css("margin",0);
            	}
            }
        },
        ...
        */
        ...
        /**
         * 清除指定的参数,即清除构建Grid时传入的参数.
         * 可以指定参数名或者参数名组成的数组
         */
        clearParams:function(paramName) {
        	var p = this.options,
        		isString = typeof paramName == 'string',
        		isArray = Object.prototype.toString.apply(paramName) === '[object Array]';
        	if (p.parms && paramName) {
                if (p.parms.length) {
                    $(p.parms).each(function (index,el) {
                    	if((isString && this.name === paramName) || (isArray && $.inArray(this.name,paramName) > -1)) {
                    		p.parms.splice(index,1);
                    	}
                    });
                } else if (typeof p.parms == "object") {
                    for (var name in p.parms) {
                    	if((isString && name === paramName) || (isArray && $.inArray(name,paramName) > -1)) {
                    		delete p.parms[name];
                    	}
                    }
                }
        ...
        updatePager:function(page) {
        	$('.pcontrol input', this.toolbar).val(page || this.opetions.pageCount);
        },
        //改变分页
        changePage: function (ctype)
        {
            ...
            // 如果本页通过全选按钮全选记录行,翻页后全选仍标记仍存在,在此将其清除
            $(".l-grid-hd-row").removeClass('l-checked');
        },
        ...
        _showData: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                if(p.page > p.pageCount) p.page = p.pageCount;
                //更新分页
                g._buildPager();
            }
            //加载中
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            if (column.render)
            {
                var realVal = column.render.call(g, rowdata, rowindex, value, column);
               	//添加判断字段长度如果长的话添加span处理 zxh
                if(realVal.search("href=")==-1&&realVal.search("src=")==-1&&realVal.search("\<\/span\>","i")==-1){
                	var showVal = realVal.length<15?realVal:realVal.substring(0,14)+"...";
                	content = "<span title='"+realVal+"'>"+realVal+"</span>";
                }else{
                	content = realVal;
                }
            }
        ...
        _onClick: function (e)
        {
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                {
                    var row = $(src.row);
                    // 如果配置了单击行时单选记录,则在选择行之前清除已选记录(后来添加 by FT.ZYH)
					if(!src.checkbox && row.attr('id').indexOf('|1|') == -1 && p.rowClickSingleSelect) {
						var allSelectedRows = g.getSelectedRows();
						for(var i=allSelectedRows.length;i--;) {
							g.unselect(allSelectedRows[i]);
						}
					} 
                ...
                if(!src.checkbox) g.trigger('rowClick',[src.data, src.data['__id'], src.row]); // 如果点击的不是checkbox,则触发rowClick事件(后来添加 by FT.ZYH)
