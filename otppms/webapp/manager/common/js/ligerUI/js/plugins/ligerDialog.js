...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        title: getLangVal('common_syntax_tip',cPath),        //头部 
    ...
    $.ligerDefaults.DialogString = {
        titleMessage: getLangVal('common_syntax_tip',cPath),                     //提示文本标题
        ok: getLangVal('common_syntax_sure',cPath),
        yes: getLangVal('common_syntax_yes',cPath),
        no: getLangVal('common_syntax_no',cPath),
        cancel: getLangVal('common_syntax_cancel',cPath),
        waittingMessage: getLangVal('common_is_waiting',cPath)
    };
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            if((p.type == "warn"||p.type == "success"||p.type == "error"||p.type == "question")){
            ...
            //设置锁定屏幕、拖动支持 和设置图片
            g.mask();
            if(isIE6){//如果是IE6 此遮罩要加IFame来遮住select
            	$('<iframe id="hiddenSelectDiv" name="hiddenSelectDiv" style="position: absolute;width: 100%;height: 100%;z-index:8000;"></iframe>').appendTo('.l-window-mask');
            }
            ...
            if (!p.fixedType)
            {
                ...
                if(isIE6){
	                //IE6需要自己加的要设置用来遮挡select的iframe 高度
	                $("#hiddenSelectDiv").css("height",g.dialog.height());
                }
        ...
        {
            var g = this, p = this.options;
            //打开帮助文档，当点击最大化时，打开一个新网页
            if(p.title.indexOf('帮助文档') < 0) {
	            if (g.winmax)
	            {
    ...
    {
        return $.ligerDialog.alert(content+"aaaa", title, 'question');
    };
