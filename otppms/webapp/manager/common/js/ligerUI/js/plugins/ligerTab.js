...
(function ($)
{
    ...
    $.ligerDefaults.TabString = {
        closeMessage: getLangVal('common_tab_close_this_page',cPath),
        closeOtherMessage: getLangVal('common_tab_close_other',cPath),
        closeAllMessage: getLangVal('common_tab_close_all',cPath),
        reloadMessage: getLangVal('common_refresh',cPath)
    };
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        setTabButton: function ()
        {
            ...
            {
				// 原$(this).width() + 2，加5的目的是：TAB选项卡标签超出最大宽度，TAB显示不全；
                sumwidth += $(this).width() + 5;
            });
        ...
        overrideTabItem: function (targettabid, options)
        {
            ...
            //如果已经存在  zxh 去掉此处 说明: 在调用覆盖方法时，tabid一样也可以覆盖，如组织机构切换只换页面，不换tabid
        ...
        addTabItem: function (options)
        {
            ...
            // 2013.8.1 zxh 扩展一个options 属性  js中在调用 addTabItem() 方法时 tab是否停留在最后一个tab
            var isToLastTabItem =  options.isToLastTabItem;// 别处的调用方法可以不传入此参数 此参数主要用于和权限有关的多tab树
            if (g.setTabButton())// 此方法返回的是 左右 按钮是否加载了
            {
            	if(isToLastTabItem!='view'){
                	g.moveToLastTabItem();
                }else{
                   // 左右切换 图标遮住了最左侧的tab
                   g.tab.links.ul.animate({ left: 12});
                }
            }
            //增加事件
