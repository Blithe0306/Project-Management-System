 ...
 * @author YXS
 */
public class ImpDomainTag extends BaseTag {

    private static final long serialVersionUID = -7366334203485555815L;
    private Logger logger = Logger.getLogger(ImpDomainTag.class);
    ...
    private String optionStr() throws Exception {
        ...
        try {
        	...
        	if(StrTool.listNotNull(domainInfos)){ //如果非空
	            DomainInfo domain = (DomainInfo)domainInfos.get(0);
	            sBuilder.append(domain.getDomainName());
	            sBuilder.append("<input name='").append("domainId").append("' "); 
                sBuilder.append("type='").append("hidden").append("' "); 
                sBuilder.append("id='").append("domainId").append("' "); 
                sBuilder.append("value='").append(domain.getDomainId()).append("' "); 
                sBuilder.append("/>");
        	}    
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
