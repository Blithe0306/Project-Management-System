package com.ft.otp.util.dyloader;
...
/**
 * 从指定目录、jar文件加载类
 * 使用：通过下面的方式扩展当前加载器 ClassLoader cld=new DynaClassLoader(Thread.currentThread().getContextClassLoader());
 *
 * @Date in Mar 28, 2013,1:40:27 PM
 *
 * @version v1.0
 *
 * @author TBM
...
public class DynaClassLoader extends ClassLoader {
    ...
    public Class<?> getDynaClass(String name, boolean b) {
        try {
            return loadClass(name, b);
    ...
    public Class<?> getDynaClass(String name, boolean b, String errmsg) throws ClassNotFoundException {
        return loadClass(name, b);
    }

    /**
     * 加载class <p/> todo 是否需做缓存、仔细仿照父loadClass的过程、使用参数b
     */
